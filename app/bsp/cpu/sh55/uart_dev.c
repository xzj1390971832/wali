...
static void UT0_isr_fun(void)
{
    ...
    if ((JL_UT0->CON & BIT(3)) && (JL_UT0->CON & BIT(14))) {
        ...
        if (uart0.kfifo.buf_size == rx_len) {
            uart0.write(uart0.kfifo.buffer,rx_len);
            //rx_len = 0;
        }
        */
        if (rx_len == 10 && read_one == 1) {


            memcpy(buffer_a, uart0.kfifo.buffer, rx_len);
            read_one = 0;
            //uart0.write(buffer_a,rx_len);
        ...
        //if(read_one == 1){
        //    buffer_a[0] = JL_UT0->BUF;
        //    uart0.write(buffer_a,1);
        //}


        /*
        if(uart0.kfifo.buffer[0] == 0xb6){
...
static void UT0_open(u32 baud, u32 is_9bit, void *cbuf, u32 cbuf_size, u32 rx_cnt, u32 ot)
{
    ...
    request_irq(IRQ_UART0_IDX, IRQ_UART0_IP, UT0_isr_fun, 3);
    if (cbuf) {
 ...
 */
void UT1_write_buf(const u8 *buf, u32 len)
{
    // log_info("enter\n");
    u32 i;
    if (len == 0) {
...
void test1(){
    ...
    arg.tx_pin = 0xff;//不使用发送功能赋值0xff
    arg.rx_pin = IO_PORTA_00;//不使接收功能赋值0xff
    arg.rx_cbuf = read_buf;  //uart0 不支持dma,支持byte接收,初始化后中断接收到该指针
    //arg.rx_cbuf = NULL;  //不初始化,则不开中断轮询接收
    arg.rx_cbuf_size = 64;//uart0
    arg.frame_length = 10;//uart0 不支持
    arg.rx_timeout = 10;  //uart0 不支持
...
void uart01_test()
{
    ...
    arg.tx_pin = IO_PORTA_11; //IO_PORTB_09;//不使用发送功能赋值0xff
