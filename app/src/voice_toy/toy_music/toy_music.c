...
void ADC_updata(){
    adc_value[0] = adc_get_value(ADC_CH_PA1);
    adc_value[1] = adc_get_value(ADC_CH_PA0);
    adc_value[2] = adc_get_value(ADC_CH_PA2);  
    // adc_value[3] = adc_get_value(ADC_CH_PA11);
}

static u16 time_cnt = 0;
static u16 music_count = 0;
...
*/
    ...
    // simple_play_file_bypath(&dec_pctl[0], "dir_song/a3.f1a");
    music_count = 1;

    /*

    gpio_set_direction(IO_PORTA_11,1); //配置输入
    gpio_set_die(IO_PORTA_11,1);            //PA1口使能数字输入
    gpio_set_pull_up(IO_PORTA_11,1);        //PA1口打开上拉

    gpio_set_direction(IO_PORTA_00,0); //配置输入
...
*/
    ...
    while (1) {
        //------------- main ----------------//
        ADC_updata();               //更新ADC数据，大约1.5ms一次循环更新一次
        // log_info("dj1:%d dj2:%d dj3:%d", adc_value[0],adc_value[1],adc_value[2]);
        // motor_head_return();
        // motor_ctrl(); 
        uart1_data_handler();
        //-----------------------------------//
